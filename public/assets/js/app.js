...
(function ($) {
    ...
    function initCustomScrollbar(type) {
        ...
        if (type == 'resize') {
            ...
            if (wind_.width() >= 768) {
				$('.chat-app .chat-sidebar .chat-sidebar-messages')[0] ? $('.chat-app .chat-sidebar .chat-sidebar-messages').scrollTop($('.chat-app .chat-sidebar .chat-sidebar-messages').get(0).scrollHeight, -1).niceScroll({railalign: "left",rtlmode: true,zindex: 2,cursorborder: "none"}) : '';

				$('.chat-app .chat-body .chat-body-messages')[0] ? $('.chat-app .chat-body .chat-body-messages').scrollTop($('.chat-app .chat-body .chat-body-messages').get(0).scrollHeight, -1).niceScroll({railalign: "left",rtlmode: true,zindex: 2,cursorborder: "none"}) : '';
            }
        }
    }
